...
export class SignupComponent implements OnInit {
  ...
  onSubmit() {
    ...
    this.auth.isEmailExist(this.emailAddress).then((data)=>{
      if(!data){
        this.auth.signupUserProfile(json).then((data) => {
          ...
          this.signUpError = "Error occured during registraion. Please try again later. Or contact info@nlstechnology.com";
          this.showLoading = false;
          this.step = -1;
          console.log(e);
        });
        ...
        this.signUpDetails_1[0].errorMesg = "Email Id already exist";
        this.signUpDetails_1[0].isValid = false;
        this.showLoading = false;
      }
    });
