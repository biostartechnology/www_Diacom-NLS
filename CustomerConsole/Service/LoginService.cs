    ...
    public class LoginService
    {
            ...
            if (!string.IsNullOrWhiteSpace(userName) && !string.IsNullOrWhiteSpace(password))
            {
                try
                {
                    ...
                    if (user != null && user.Count > 0)
                    {
                        ...
                        {
                            return ResponseModel.getResponse(false, "1", 0);
                        }
                    }
                }
                catch (Exception e)
        ...
        internal static string SetNewPassword(string id, string newPassword)
        {
            if (!string.IsNullOrWhiteSpace(id) && !string.IsNullOrWhiteSpace(newPassword))
            {
                try
                {
                    ...
                    {
                        newPassword = Utility.OneWayEncryptString(newPassword);
        ...
        private static void SendForgotPasswordAlertMail(ForgotPasswordModel forgotPasswordModel)
        {
            try
            {
                ...
                Settings.mailUtility.Send("Password Reset", html, forgotPasswordModel.EmailId, "", true);
            }
            catch (Exception e)
            {
                Logger.Log(e);
